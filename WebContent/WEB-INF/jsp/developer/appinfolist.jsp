...
<div class="row">
	<div class="col-md-12">
		<div class="x_panel">
			...
			<div class="x_content">
				<form method="post"
					action="${pageContext.request.contextPath}/dev/app/list">
							...
							<div class="form-group">
								...
								<div class="col-md-6 col-sm-6 col-xs-12">
									<input name="querySoftwareName" type="text"
										...
										value="${queryAppInfoVO.querySoftwareName }">
								</div>
							</div>
						</li>
									...
									<select name="queryStatus" class="form-control">
										<c:if test="${statusList != null }">
											...
											<c:forEach var="dataDictionary" items="${statusList}">
												<option
													<c:if test="${dataDictionary.valueId == queryAppInfoVO.queryStatus }">selected="selected"</c:if>
									...
									<select name="queryFlatformId" class="form-control">
										<c:if test="${flatFormList != null }">
											...
											<c:forEach var="dataDictionary" items="${flatFormList}">
												<option
													<c:if test="${dataDictionary.valueId == queryAppInfoVO.queryFlatformId }">selected="selected"</c:if>
													value="${dataDictionary.valueId}">${dataDictionary.valueName}</option>
											</c:forEach>
										</c:if>
									</select>
									...
									<select id="queryCategoryLevel1" name="queryCategoryLevel1"
										...
										<c:if test="${categoryLevel1List != null }">
											...
											<c:forEach var="appCategory" items="${categoryLevel1List}">
												<option
													<c:if test="${appCategory.id == queryAppInfoVO.queryCategoryLevel1 }">selected="selected"</c:if>
									...
									<input type="hidden" name="categorylevel2list"
										...
										<c:if test="${categoryLevel2List != null }">
											...
											<c:forEach var="appCategory" items="${categoryLevel2List}">
												<option
													<c:if test="${appCategory.id == queryAppInfoVO.queryCategoryLevel2 }">selected="selected"</c:if>
													value="${appCategory.id}">${appCategory.categoryName}</option>
											</c:forEach>
										</c:if>
									</select>
									...
									<select name="queryCategoryLevel3" id="queryCategoryLevel3"
										...
										<c:if test="${categoryLevel3List != null }">
											...
											<c:forEach var="appCategory" items="${categoryLevel3List}">
												<option
													<c:if test="${appCategory.id == queryAppInfoVO.queryCategoryLevel3 }">selected="selected"</c:if>
													value="${appCategory.id}">${appCategory.categoryName}</option>
											</c:forEach>
										</c:if>
									</select>
				...
				<div class="row">
					...
					<a href="${pageContext.request.contextPath}/dev/flatform/app/appinfoadd" class="btn btn-success btn-sm">新增APP基础信息</a>
                      ...
                      <ul class="dropdown-menu" role="menu">
                        ...
                        <li><a  class="modifyAppInfo" <%-- href="${pageContext.request.contextPath}/dev/app/appinfomodify" --%>
						...
						<li><a  class="deleteApp" appinfoid=${appInfo.id }  appsoftwarename=${appInfo.softwareName } data-toggle="tooltip" data-placement="top" title="" data-original-title="删除APP基础信息以及全部版本信息">删除</a></li>
								...
								<tbody>
									<c:forEach var="appInfo" items="${pageBean.result }"
										varStatus="status">
										<tr role="row" class="odd">
											<td tabindex="0" class="sorting_1">${appInfo.softwareName}</td>
											<td>${appInfo.APKName }</td>
